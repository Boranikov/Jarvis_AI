...
def read_file(name: str, content: str, path: Optional[str] = None,) -> str | bool:
    """Belirtilen dosyanın içeriğini okur ve string olarak geri döndürür"""
    target = _resolve_target(name, content, path, "read_file")
    if not target:
...
def write_to_file(name: str, content: str, path: Optional[str] = None) -> bool:
    ...
    target = _resolve_target(name, content, path, "write_to_file")
    if not target:
        return False
